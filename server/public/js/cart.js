...
const deleteItemFromCart = (productId) => {
  ...
  let deletedItemPrice =
    cartCost - cartItems[productId].inCart * cartItems[productId].price;
...
const changeProductCountInSpan = (product) => {
  document.getElementById("productCountInCart" + product._id).innerHTML =
    product.inCart;
  document.getElementById("productPriceInCart" + product._id).innerHTML = `$${
    product.inCart * product.price
  },00`;
  document.getElementById(
    "totalPriceOfItems"
  ).innerHTML = `$${localStorage.getItem("totalCost")},00`;
};
    ...
    Object.values(cartItems).map((item) => {
      productsContainer.innerHTML += `
            <div class="row">
                ...
                <input type="button" onclick="deleteItemFromCart('${
                  item._id
                }')" value="delete">
            ...
            <input type="button" onclick="incrementItemCount('${
              item._id
            }')" value="+">
            <p id="productCountInCart${item._id}">${item.inCart}</p>
            <input type="button" onclick="decrementItemCount('${
              item._id
            }')" value="-">
            </div>
            <div class="total col-md-3">
            <p id="productPriceInCart${item._id}">$${
